...
def login(user_name, password):
    ...
    if user_info == "None":
        #msg = Label(label_name, text="No such user")
        #msg.place(x=20, y=150)
        return False
    else:
        if user_info[1] == password:
            ...
            #msg = Label(label_name, text="Password not correct")
            #msg.place(x=20, y=150)
            return False
...
def return_car(order_id, car_end_location):
    ...
    4. 根据 car_id 获取：CarPower，按照 CarPower - hour*10 的方式计算 left_power，不满 1h 按 1h 计算，left_power 最小为 0
...
def update_car(update_action, car_id, car_location = "Main Building"):
    '''
    ...
    if update_action == "charge":
        ...
        SqlFunction.update_car_state(car_id, "avaliavle")
        return True
